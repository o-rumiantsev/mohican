...
MySQLProvider.prototype.delete = function(entry, callback) {
  const query = queries.del(entry);
  this.connection.query(query, err => callback(err));
};

MySQLProvider.prototype.update = function(entry, callback) {
  const query = queries.update(entry);
  this.connection.query(query, err => callback(err));
};

MySQLProvider.prototype.drop = function(category, callback) {
