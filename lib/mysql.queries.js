...
const createTable = scheme => {
  ...
  for (const field in scheme) {
    ...
    if (Array.isArray(options)) {
      ...
      // TODO: Fix foreign key definition
      options = `FOREIGN KEY (reference ${reference})`;
    } else {
      options = '';
    }
  ...
  const query = `CREATE TABLE ${table} (${fields})`;
  return query;
};
...
const createTableIfNotExists = entry => {
  ...
  const query = createTable(scheme)
    .replace('CREATE TABLE', REPLACE_MATCH + ' IF NOT EXISTS');
